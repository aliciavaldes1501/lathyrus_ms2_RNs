...
load("C:/Users/alici/Desktop/large_objects/large_objects_3.RData")
load("C:/Users/alici/Desktop/large_objects/large_objects_4.RData")
load("C:/Users/alici/Desktop/large_objects/large_objects_ndayszero.RData")
```
...
# Additional analyses (response letter)
...
```{r}
data_weather_var<-data_weather%>%filter(year==1987|year==1988|year==1989|
...
```{r}
datadef_var<-full_join(datadef,data_weather_var)%>%
...
## Analyses with n days below zero
...
```{r}
datadef_ndayszero<-full_join(datadef,n_days_min_below_zero)%>%
  mutate(n_days_min_below_zero_c=scale(n_days_min_below_zero,center=T,scale=F))
```

```{r eval=FALSE, include=FALSE}
univar.FFD_yearonly.all.brm_ndayszero<-brm(formula=FFD~n_days_min_below_zero_c+
                                             (1|year),
                                           data=datadef_ndayszero,
                                     warmup=1000,iter=4000,thin=2,chains=4,
                                     # 4 chains, each with 4000 iterations
                                     inits="random",seed=12345,cores=my.cores)
# Total of 6000 post-warmup samples
...
```{r}
summary(univar.FFD_yearonly.all.brm_ndayszero)
```

```{r eval=FALSE, include=FALSE}
univar.FFD.all.brm_ndayszero<-brm(formula=FFD~n_days_min_below_zero_c+
                                    (1|year)+(1|id),
                                  data=datadef_ndayszero,
                            warmup=1000,iter=4000,thin=2,chains=4,
                            # 4 chains, each with 4000 iterations
...
```{r }
summary(univar.FFD.all.brm_ndayszero)
```

```{r eval=FALSE, include=FALSE}
univar.FFD_RR.all.brm_ndayszero<-brm(formula=FFD~n_days_min_below_zero_c+
                                       (1|year)+(n_days_min_below_zero_c|id),
                                     data=datadef_ndayszero,
                           warmup=1000,iter=4000,thin=2,chains=4,
                           inits="random",seed=12345,cores=my.cores,
                           sample_prior="yes")
...
```{r}
summary(univar.FFD_RR.all.brm_ndayszero) 
```
```{r eval=FALSE, include=FALSE}
univar.FFD_yearonly.all.brm_ndayszero <- add_criterion(univar.FFD_yearonly.all.brm_ndayszero,"loo")
univar.FFD.all.brm_ndayszero <- add_criterion(univar.FFD.all.brm_ndayszero,"loo")
univar.FFD_RR.all.brm_ndayszero <- add_criterion(univar.FFD_RR.all.brm_ndayszero,"loo")
```

```{r}
loo_comp_ndayszero<-loo_compare(univar.FFD_yearonly.all.brm_ndayszero,
                                univar.FFD.all.brm_ndayszero,
                                univar.FFD_RR.all.brm_ndayszero,
                                criterion="loo")
```
...
```{r}
bayes_R2(univar.FFD_yearonly.all.brm_ndayszero)
bayes_R2(univar.FFD.all.brm_ndayszero)
bayes_R2(univar.FFD_RR.all.brm_ndayszero)
```

```{r}
BLUPs_MCMC.all.brms_ndayszero  <- cbind(as.factor(c(1:837)),
                                        as.data.frame(ranef(
                                          univar.FFD_RR.all.brm_ndayszero)$id)
                                        [c(1:2,5:6)])
colnames(BLUPs_MCMC.all.brms_ndayszero) <- c("id", "intercept_ndayszero",
                                             "intercept_sd_ndayszero",
                                             "slope_ndayszero",
                                             "slope_sd_ndayszero")
with(BLUPs_MCMC.all.brms_ndayszero,cor(intercept_ndayszero,slope_ndayszero)) 
# highly correlated!
```

```{r}
datadef_ndayszero<-datadef_ndayszero%>%
  left_join(BLUPs_MCMC.all.brms_ndayszero%>%
              dplyr::select(intercept_ndayszero,intercept_sd_ndayszero,
                            slope_ndayszero,slope_sd_ndayszero)%>%
              rownames_to_column(var="id"))
```

```{r}
predict_id_ndayszero<-ggpredict(univar.FFD_RR.all.brm_ndayszero,
                                terms=c("n_days_min_below_zero_c","id"),
                                type="random")
predict_mean_ndayszero<-ggpredict(univar.FFD_RR.all.brm_ndayszero,
                                  terms=c("n_days_min_below_zero_c"),
                                  type="random")
```

```{r fig.height=10, fig.width=12}
data_fig1_ndayszero<-full_join(data.frame(predict_id_ndayszero),
          datadef_ndayszero%>%dplyr::select(id,slope_ndayszero)%>%group_by(id)%>%
            summarise(slope_ndayszero=mean(slope_ndayszero))%>%rename(group=id))
ggplot()+
  geom_line(data=data_fig1_ndayszero,
            aes(x=x,y=predicted,group=group,color=slope_ndayszero),
            size=0.01,alpha=0.7)+
  scale_color_viridis(end=0.9,option = "C")+
  geom_line(data=data.frame(predict_mean_ndayszero),aes(x=x,y=predicted),
            color="black",size=1)+
  my_theme()+
  xlab("Number of days below zero in April (mean-centered)")+
  ylab("Predicted FFD")
```

## Figure reponse letter

```{r}
grid.arrange(
  ggplot()+
    geom_line(data=data_fig1,
              aes(x=x,y=predicted,group=group,color=slope),
              size=0.01,alpha=0.7)+
    scale_color_viridis(end=0.9,option = "C")+
    geom_line(data=data.frame(predict_mean_2),aes(x=x,y=predicted),
              color="black",size=1)+
    my_theme()+scale_y_continuous(limits=c(45,65))+
    xlab("April temperature (mean-centered)")+
    ylab("Predicted FFD"),
  ggplot()+
    geom_line(data=data_fig1_var,
...
```{r eval=FALSE, include=FALSE}
univar.FFD_RR.all.brm_periods<-brm(formula=FFD~cmean_4*period,
                           data=datadef,
                           warmup = 1000,iter = 4000,thin=2,chains=4,
                           inits = "random",seed = 12345,cores = my.cores,
...
```{r}
grid.arrange(
  ggplot(as.data.frame(conditional_effects(
    univar.FFD_RR.all.brm_periods)$'cmean_4'),
    aes(x=cmean_4,y=estimate__,ymin=lower__,ymax=upper__))+
    geom_ribbon(alpha=0.3)+geom_line()+
    xlab("April temperature (mean-centered)")+ylab("Predicted FFD")+
    my_theme(),
  ggplot(as.data.frame(conditional_effects(
    univar.FFD_RR.all.brm_periods)$'period'),
    aes(x=period,y=estimate__,ymin=lower__,ymax=upper__))+
    geom_errorbar()+geom_point(size=2)+
    xlab("Period")+ylab("Predicted FFD")+
    my_theme(),
  ggplot(as.data.frame(conditional_effects(
    univar.FFD_RR.all.brm_periods)$'cmean_4:period'),
    aes(x=cmean_4,y=estimate__,ymin=lower__,ymax=upper__))+
    geom_ribbon(aes(fill=period),alpha=0.3)+geom_line(aes(color=period),size=1)+
    xlab("April temperature (mean-centered)")+ylab("Predicted FFD")+
    my_theme(),
  ncol=3)
```
