...
load("output/univar.FFD_RR.all.brm.brm.RData")
...
plot(univar.FFD_RR.all_intslope)
...
```{r BLUPs_all20, echo=FALSE, fig.height=8, fig.width=8}
...
conditional_effects(univar.FFD.all.brm)
```
...
conditional_effects(univar.FFD_RR.all.brm)
```

#### Extract BLUPs from this model

Check that this is the correct way!
...
kable(rbind(MCMCglmm=summary(univar.FFD_RR.all$VCV)$statistics[,1],
...
par(mfrow = c(1,2))
plot(density(S.bivar1.all[,1]), main = "S_intercepts")
plot(density(S.bivar1.all[,2]), main = "S_slopes")
...
par(mfrow = c(1,2))
plot(density(beta_post_bivar1.all[,1]), main = "beta_intercepts")
plot(density(beta_post_bivar1.all[,2]), main = "beta_slopes")
```
...
# Plot posterior distribution of selection differentials
par(mfrow = c(1,2))
plot(density(S.bivar2.all[,1]), main = "S_intercepts")
plot(density(S.bivar2.all[,2]), main = "S_slopes")
...
# Plot posterior distribution of selection gradients
par(mfrow = c(1,2))
plot(density(beta_post_bivar2.all[,1]), main = "beta_intercepts")
plot(density(beta_post_bivar2.all[,2]), main = "beta_slopes")
```
...
# Plot posterior distribution of selection differentials
par(mfrow = c(1,2))
plot(density(S.bivar3.all[,1]), main = "S_intercepts")
plot(density(S.bivar3.all[,2]), main = "S_slopes")
...
# Plot posterior distribution of selection gradients
par(mfrow = c(1,2))
plot(density(beta_post_bivar3.all[,1]), main = "beta_intercepts")
plot(density(beta_post_bivar3.all[,2]), main = "beta_slopes")
```
...
# Plot posterior distribution of selection differentials
par(mfrow = c(1,2))
plot(density(S.bivar4.all[,1]), main = "S_intercepts")
plot(density(S.bivar4.all[,2]), main = "S_slopes")
...
# Plot posterior distribution of selection gradients
par(mfrow = c(1,2))
plot(density(beta_post_bivar4.all[,1]), main = "beta_intercepts")
plot(density(beta_post_bivar4.all[,2]), main = "beta_slopes")
```
...
Regarding distributions, I tried Poisson distribution for fitness, but not sure how eventual overdispersion is handled. I also tried adding and observation-level random effect, and using a negative binomial distribution. Results seem quite similar.
...
conditional_effects(bivar1.all.brm.pois)
```
...
plot(bivar1.all.brm.OLRE)
conditional_effects(bivar1.all.brm.OLRE)
```
...
conditional_effects(bivar1.all.brm.nb)
```
...
conditional_effects(bivar2.all.brm.nb)
```
...
conditional_effects(bivar3.all.brm.nb)
```
...
conditional_effects(bivar4.all.brm.nb)
```
